...
from app.db import connect_direct, log_collection, upsert_observations_batch, upsert_stop
...
def collect_daily(
    db_path: str, api_url: str, api_key: str, stop_id: str, service_date: str | None = None
) -> dict:
    ...
    try:
        ...
        if all_empty:
            log_collection(db, stop_id, "daily", target_date, departures_found=0, no_service=1)
            db.close()
            return {
                "status": "no_service",
                "date": target_date,
        ...
        log_collection(db, stop_id, "daily", target_date, departures_found=len(observations))

        logger.info("Collected %d departures for %s on %s", len(observations), stop_id, target_date)
        db.close()
...
def poll_realtime_once(db_path: str, api_url: str, api_key: str, stop_id: str) -> dict:
    ...
    try:
        ...
        logger.info(
            "Realtime poll: %d departures (%d with realtime) for %s",
            len(observations),
            realtime_count,
            stop_id,
        )
        db.close()
        return {"status": "ok", "updated": len(observations), "with_realtime": realtime_count}
